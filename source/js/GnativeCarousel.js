export default class GnativeCarousel {
	...
	getPropertyOfElement(element, property, howCalled = 'none') {
    // console.log(howCalled, element, element.style[property])
		...
		if (final.breakpoints !== undefined) {
			...
			for (let i = arrOfPoints.length - 1; i >= 0; i--) {
				if (i === arrOfPoints.length - 1) {
					final.breakpoints[arrOfPoints[i]].adaptive = final.breakpoints[arrOfPoints[i]].adaptive ? final.breakpoints[arrOfPoints[i]].adaptive : false
	...
	setResponsiveOptions() {
    console.log(this.finalSettings.breakpoints, 'setResponsiveOptions')
	...
	makeBoxSizing() {
		if (this.getPropertyOfElement(this.sliderContainer, 'box-sizing', 'makeBoxSizing, el: sliderContainer') !== 'border-box')
	...
	setSliderHeight() {
		const getBiggestInnerElement = (...elems) => {
			...
			elems.forEach(nodeElem => {
				if (this.isNode(nodeElem)) {
					const boxShadow = isNaN(parseInt(this.getPropertyOfElement(nodeElem, 'box-shadow', 'setSliderHeight, el: static or items').split('px ')[2]) * 2) ?
						0 :
						parseInt(this.getPropertyOfElement(nodeElem, 'box-shadow', 'setSliderHeight, el: static or items').split('px ')[2]) * 2

					const height = parseInt(this.getPropertyOfElement(nodeElem, 'height', 'setSliderHeight, el: static or items'))
		...
		const sliderPaddingY = parseInt(this.getPropertyOfElement(this.sliderContainer, 'padding-top', 'setSliderHeight 1'))
			+ parseInt(this.getPropertyOfElement(this.sliderContainer, 'padding-bottom'))
		const sliderBorderY = parseInt(this.getPropertyOfElement(this.sliderContainer, 'border-top', 'setSliderHeight 2'))
			+ parseInt(this.getPropertyOfElement(this.sliderContainer, 'border-bottom'))
		const calculatedHeight = parseInt(this.getPropertyOfElement(this.sliderContainer, 'height', 'setSliderHeight 3'))
		...
		const biggestElem = getBiggestInnerElement(this.staticItem, this.items[0], 'setSliderHeight')

    console.log(this.responsiveOptions, 'sliderHeight')
	...
	setSliderWidth() {
    const sumWidthOfStaticItem = 0
    //static item
    if(this.isNode(this.staticItem)){
      const staticItemWidth = parseInt(this.getPropertyOfElement(this.staticItem, 'width', 'setSliderWidth 1'))
      const staticItemBoxShadow = isNaN(parseInt(this.getPropertyOfElement(this.staticItem, 'box-shadow', 'setSliderWidth 2').split('px ')[2]) * 2) ?
        0 :
        parseInt(this.getPropertyOfElement(this.staticItem, 'box-shadow', 'setSliderWidth 3').split('px ')[2]) * 2
      const sumWidthOfStaticItem = staticItemWidth + staticItemBoxShadow
		...
		let itemsContainerWidth = itemWidth / 100 * 10 + this.items[0].getBoundingClientRect().width / 2
  ...
  //todo не правильно сделан адаптив, в функции setResponsiveOptions
  //нужна функция для определения наличия статик айтема и баттонов

	setParentHeight() {
		if (this.getPropertyOfElement(this.sliderContainer, 'position', 'setParentHeight 1') === 'absolute') {
			const heightOfParent = this.getPropertyOfElement(this.sliderContainer.parentNode, 'height', 'setParentHeight 2')
	...
	setTableOfPositions() {
		...
		for (let i = mainElement - 1; i >= 0; i--) {
			...
			if (mainElement - 1 === i) {
				...
				left = this.tableOfPositions[mainElement].left - (width / 100 * 10)
				invert = 0.05
			}
		...
		for (let i = mainElement + 1; i < this.items.length; i++) {
			...
			if (mainElement + 1 === i) {
				width = this.tableOfPositions[mainElement].width - (this.tableOfPositions[mainElement].width / 100 * 15)
				height = this.tableOfPositions[mainElement].height - (this.tableOfPositions[mainElement].height / 100 * 15)
				left = this.tableOfPositions[mainElement].left + this.tableOfPositions[mainElement].width - (width / 100 * 90)
				invert = 0.1
			}
			else if (mainElement + this.responsiveOptions.itemsOnSide >= i) {
				width = this.tableOfPositions[i - 1].width - (this.tableOfPositions[i - 1].width / 100 * 5)
